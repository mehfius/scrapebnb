...
// Constrói a requisição GraphQL para a API
const buildGraphQLRequest = (locationString, checkin, checkout, adults = 1, cursor = null) => {
    const rawParams = [
...
// Busca e concatena os resultados de todas as páginas da API
...
// O manipulador principal da função serverless
export default async function handler(req, res) {
    ...
    try {
        ...
        const graphqlRequest = buildGraphQLRequest(location, checkin, checkout, adults);
        ...
        const combinedResults = allSearchResults.map((listing, index) => {
            const mapInfo = allMapResults[index];
            return {
                title: listing.title,
                room: mapInfo ? mapInfo.listingId : null,
                host: listing.passportData ? listing.passportData.userId : null,
                picture: listing.contextualPictures?.[0]?.picture || null
            };
        });
        ...
        await fs.writeFile(filepath, JSON.stringify(combinedResults, null, 2));
