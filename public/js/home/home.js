        ...
        if (jobs.length > 0) {
            ...
            jobs.forEach(job => {
                const option = jte({
                    tag: 'option',
                    value: job.id,
                    // MODIFIED: Directly using job.qtd in innerhtml
                    innerhtml: `${new Date(job.created_at).toLocaleString()} | Adults: ${job.adults || 'N/A'} | Min Bedrooms: ${job.min_bedrooms || 'N/A'} | Qtd: ${job.qtd || 'N/A'}` // <--- 'job.qtd' used here
                });
